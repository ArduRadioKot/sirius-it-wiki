...
Workflow `.github/workflows/schedule.yml` refreshes the snapshot every 2 hours (and on manual *Run workflow*) with `scripts/publish-schedule.sh`. It commits to `main` only when lessons change or the snapshot is older than 12 hours, and GitHub Pages rebuilds from that commit. If the university site is unreachable from GitHub, the run keeps the old snapshot and commits nothing. Scheduled runs can lag, and GitHub pauses them after 60 days without repository activity. The app first tries to collect the timetable from the university on the user's device whenever you open it, return to the tab, come back online, or after four hours in the background, then falls back to the published snapshot.
